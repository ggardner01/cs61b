...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        if(leftIndex(index) > size){
            return;
        } else if (rightIndex(index) > size){
            if(getNode(leftIndex(index)).myPriority < getNode(index).myPriority ){ swap(index,leftIndex(index));}
            return;
        }

        if(getNode(index).myPriority > Math.min( getNode(leftIndex(index)).myPriority , getNode(rightIndex(index)).myPriority )){
            if(getNode(leftIndex(index)).myPriority > getNode(rightIndex(index)).myPriority ){
